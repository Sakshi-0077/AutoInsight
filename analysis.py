...
def handle_missing_values(df):
    ...
    remaining_missing_rows = df.isnull().any(axis=1)
    ...
    if final_rows_dropped > 0:
        df = df.loc[~remaining_missing_rows]

    missing_report = {
        "columns_dropped_missing": columns_to_drop,
        "rows_dropped_missing": (
            len(rows_to_drop) + final_rows_dropped
        ),
    }
...
def check_dataset_sufficiency(df):
    ...
    if len(df.columns) == 0:
        issues.append("Dataset contains no usable columns.")

    if len(df) > 0 and len(df) < MIN_ROWS_FOR_RELIABLE_STATS:
        warnings.append(
            f"Only {len(df)} rows remain. Statistical analysis may be "
            f"less reliable with fewer than {MIN_ROWS_FOR_RELIABLE_STATS} rows."
        )
    ...
    if not numerical and not categorical:
        issues.append("No supported data columns were found.")

    if df.isnull().sum().sum() > 0:
        issues.append("Missing values remain after cleaning.")

    if df.duplicated().sum() > 0:
        issues.append("Duplicate rows remain after cleaning.")
...
def clean_data(df):
    ...
    df, constant_columns_removed = remove_constant_columns(df)
    ...
    report = {
        ...
        "duplicates_found": original_duplicates,
        "duplicates_removed": duplicates_removed,
        "columns_dropped_missing": (
            missing_report["columns_dropped_missing"]
        ),
        "rows_dropped_missing": (
            missing_report["rows_dropped_missing"]
        ),
...
def get_chartable_columns(df):
    ...
    for column in numerical:
        ...
        if _looks_like_calendar_year_column(df, column):
            calendar_year_numeric.append(column)

        elif df[column].nunique(
            dropna=True
        ) <= MIN_UNIQUE_FOR_OUTLIER_CHECK:
            discrete_numeric.append(column)
    ...
    for column in categorical:
        ...
        if (
            ...
            categorical_chartable.append(column)
...
def create_visualizations(df):
    ...
    if len(continuous_numeric) >= 2:
        max_scatter_columns = continuous_numeric[:5]

        for i in range(len(max_scatter_columns)):
            for j in range(i + 1, len(max_scatter_columns)):
